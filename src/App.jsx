            ...
            {!showReportForm ? (
              ...
              <div style={{ borderTop:"1px solid var(--border)", paddingTop:20 }}>
                ...
                <button className="btn btn-red btn-full" onClick={async () => {
                  ...
                  setReports(r => [{ id:data?.id||Date.now(), emoji:rf.animal||"🐾", title:{en:rf.title,tr:rf.title}, desc:{en:rf.desc||"",tr:rf.desc||""}, location:rf.location, time:{en:"Just now",tr:"Az önce"}, status:"active", reporter:myName, volunteers:[] }, ...r]);
                  setRf({ title:"", location:"", desc:"", type:"Injured", animal:"" }); setPhoto(null); setShowReportForm(false);
                ...
                {ETA_OPTIONS.map(opt => (
                  <button key={opt.label} className="eta-btn" onClick={async () => {
                    ...
                    setReports(prev => prev.map(r =>
                      r.id === etaFor.id
                        ? { ...r, volunteers:[...(r.volunteers||[]), { name:myName, eta:opt.label, etaOrder:opt.order }] }
                        : r
                    ));
                    setEtaFor(null);
              ...
              {helpProof ? (
                <>
                  ...
                  <button className="btn btn-dark btn-full" style={{ marginBottom:8 }} onClick={async () => {
                    await db.from("reports").update({ status:"helped" }).eq("id", helpedFor.id);
                    setReports(p => p.map(x => x.id === helpedFor.id ? { ...x, status:"helped" } : x));
